 ...
 * @param {string} opts.coin[='colx'] - The coin for this wallet (colx).
 ...
 * @param {string} opts.coin[='colx'] - The expected coin for this wallet (colx).
...
WalletService.prototype.savePreferences = function(opts, cb) {
  ...
  var preferences = [{
    ...
    name: 'unit',
    isValid: function(value) {
      return _.isString(value) && _.contains(['colx'], value.toLowerCase());
    },
  }];
 ...
 * @param {Object} opts
 * @param {String} [opts.coin='colx'] (optional)
 ...
 * @param {Object} opts
 * @param {string} [opts.coin = 'colx'] - The coin to estimate fee levels from.
...
WalletService.prototype._selectTxInputs = function(txp, utxosToExclude, cb, isInstantSend) {
  ...
  function select(utxos, cb) {
    ...
    if (totalValueInUtxos < txpAmount) {
      self.logi('Total value in all utxos (' + Utils.formatAmountInColx(totalValueInUtxos) + ') is insufficient to cover for txp amount (' + Utils.formatAmountInColx(txpAmount) + ')');
      return cb(Errors.INSUFFICIENT_FUNDS);
    }
    if (netValueInUtxos < txpAmount) {
      self.logi('Value after fees in all utxos (' + Utils.formatAmountInColx(netValueInUtxos) + ') is insufficient to cover for txp amount (' + Utils.formatAmountInColx(txpAmount) + ')');
    ...
    self.logi('Big input threshold ' + Utils.formatAmountInColx(bigInputThreshold));
    ...
    _.each(smallInputs, function(input, i) {
      ...
      //log.debug('The input contributes ' + Utils.formatAmountInColx(netInputAmount));
      ...
      //log.debug('Tx size: ' + Utils.formatSize(txpSize) + ', Tx fee: ' + Utils.formatAmountInColx(fee));
      ...
      if (!_.isEmpty(bigInputs)) {
        ...
        if (feeVsAmountRatio > Defaults.UTXO_SELECTION_MAX_FEE_VS_TX_AMOUNT_FACTOR) {
          ...
          //log.debug('Fee/Single-input fee: ' + Utils.formatRatio(feeVsSingleInputFeeRatio) + ' (max: ' + Utils.formatRatio(Defaults.UTXO_SELECTION_MAX_FEE_VS_SINGLE_UTXO_FEE_FACTOR) + ')' + ' loses wrt single-input tx: ' + Utils.formatAmountInColx((selected.length - 1) * feePerInput));
      ...
      //log.debug('Cumuled total so far: ' + Utils.formatAmountInColx(total) + ', Net total so far: ' + Utils.formatAmountInColx(netTotal));

      if (netTotal >= txpAmount) {
        var changeAmount = Math.round(total - txpAmount - fee);
        //log.debug('Tx change: ', Utils.formatAmountInColx(changeAmount));

        var dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, Bitcore.Transaction.DUST_AMOUNT);
        if (changeAmount > 0 && changeAmount <= dustThreshold) {
          //log.debug('Change below dust threshold (' + Utils.formatAmountInColx(dustThreshold) + '). Incrementing fee to remove change.');
    ...
    if (netTotal < txpAmount) {
      //log.debug('Could not reach Txp total (' + Utils.formatAmountInColx(txpAmount) + '), still missing: ' + Utils.formatAmountInColx(txpAmount - netTotal));
  ...
  //log.debug('Selecting inputs for a ' + Utils.formatAmountInColx(txp.getTotalAmount()) + ' txp');
  ...
  self._getUtxosForCurrentWallet({}, function(err, utxos) {
    ...
    async.whilst(function() {
      ...
      select(candidateUtxos, function(err, selectedInputs, selectedFee) {
        ...
        //log.debug('Fee for this selection: ' + Utils.formatAmountInColx(fee));
      ...
      if (!err) {
        ...
        self.logi('Successfully built transaction. Total fees: ' + Utils.formatAmountInColx(txp.fee) + ', total change: ' + Utils.formatAmountInColx(change));
      } else {
 ...
 * @param {Object} opts
 * @param {string} [opts.coin = 'colx'] - The coin for this transaction.
